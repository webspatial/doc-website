...
function CategoryLinkLabel({
  ...
  const icon = renderDocsCategoryIcon({
  ...
  if (icon) {
    return (
      <span className={clsx(styles.categoryLinkLabel, styles.categoryLabelWithIcon)}>
        {icon}
        <span title={label} className={styles.categoryText}>
          {label}
        </span>
      </span>
    );
      ...
      <div
        ...
        <Link
          ...
          <CategoryLinkLabel
            label={label}
            level={level}
            iconHref={getCategoryIconHref(item)}
          />
        </Link>
        {href && collapsible && (
          <CollapseButton
