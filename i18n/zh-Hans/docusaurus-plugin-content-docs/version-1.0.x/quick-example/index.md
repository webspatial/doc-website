...
- After you have completed the demo, you can dive into [the real development guide](../development-guide/index.md).
...
:::tip
See [what kinds of web projects are considered WebSpatial-supported](../development-guide/web-projects-that-support-webspatial/index.md) for details.
:::
...
First modify `tsconfig.app.json` and `tsconfig.node.json`, adding the configuration that [affects JSX compilation](../development-guide/enabling-webspatial-in-web-projects/step-3-integrate-webspatial-sdk-into-web-build-tools/configure-js-ts-compiler.md).
...
Run [the Dev Server](../development-guide/enabling-webspatial-in-web-projects/step-3-integrate-webspatial-sdk-into-web-build-tools/generate-a-webspatial-specific-website.md#regular-dev-server) (keep it running in the following steps):
...
Before using [the packaging tool for WebSpatial](../development-guide/enabling-webspatial-in-web-projects/step-2-add-build-tool-for-packaged-webspatial-apps/index.md), install Xcode and the visionOS simulator:
...
Run the [development command (`run`)](../development-guide/enabling-webspatial-in-web-projects/step-2-add-build-tool-for-packaged-webspatial-apps/index.md#run) of the packaging tool:
...
Create a incomplete Web App Manifest file (this only works with the `webspatial-builder run` command, you'll need to [complete it before running on a real device or distributing it](../development-guide/enabling-webspatial-in-web-projects/prerequisite-become-a-minimal-pwa/index.md)).
...
Edit the `main.tsx` file to [add a special classname to the `<html>` element](../development-guide/enabling-webspatial-in-web-projects/step-3-integrate-webspatial-sdk-into-web-build-tools/check-if-running-in-webspatial-mode.md#css-solution) when this web code is executed as a WebSpatial app.
...
At the end of `src/index.css`, add code that sets the scene background to a [fully transparent material](../development-guide/using-the-webspatial-api/add-material-backgrounds.md) (using `--xr-background-material` from the WebSpatial API):
...
In `src/App.tsx`, mark the description text inside `count-card` as a [spatialized HTML element](../core-concepts/spatialized-elements-and-3d-container-elements.md).
...
For the [styles](../development-guide/enabling-webspatial-in-web-projects/step-3-integrate-webspatial-sdk-into-web-build-tools/check-if-running-in-webspatial-mode.md#css-solution) in `src/index.css`, move and rotate `link-card` along the Z axis (around the X axis):
